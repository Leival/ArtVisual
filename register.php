...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
    ...
    if (empty($username) || empty($email) || empty($password) || empty($cpassword)) {
        ...
        $stmt = $conn->prepare("SELECT id_artista FROM artistas WHERE email = ?");
        if ($stmt) {
            ...
            if ($stmt->num_rows > 0) {
                ...
                $insert_stmt = $conn->prepare("INSERT INTO artistas (username, email, password_hash) VALUES (?, ?, ?)");
                if ($insert_stmt) {
    ...
    <div class="container">
        <form action="" method="POST" class="login-email">
            ...
            <div class="input-group">
                <input type="text" placeholder="Nombre de Artista" name="username" 
